...
if ( isset($_POST['simpanrange']) ) {
  ...
  mysqli_query($con, "DELETE FROM range_2017") or die(mysqli_error($con));
  ...
  for ( $i=1; $i<=$jumlah; $i++ ) {
    ...
    mysqli_query($con, "INSERT INTO range_2017 (jumlah_pasien, range_awal, range_akhir) VALUES ('$jumlahpasien', '$rangeawal', '$rangeakhir')") or die (mysqli_error($con));
  }
  echo "
    <script>
  ...
  mysqli_query($con, "DELETE FROM rand_2017") or die (mysqli_error($con));
  $jumlah = $_POST['jumlah'];
  for ( $i=1; $i<=$jumlah; $i++ ) {
    ...
    mysqli_query($con, "INSERT INTO rand_2017 (nomor, id_bulan, bilangan_acak) VALUES ('$nomor', '$id_bulan', '$rand') ") or die(mysqli_error($con));
  }
  echo "<script>
